...
\begin{tikzpicture}[>=latex, scale=1.0, every node/.style={font=\small}]
  ...
  % Tre ben (kobling fra sete til gulv)
  ...
  % Etiketter under benene
  \node[s1color, font=\bfseries, below, align=center, text width=3.2cm] at (-2.4,-0.1)
    {\SOne\\ Decision rights\\ \footnotesize\itshape (hvem beslutter)};
  \node[s2color, font=\bfseries, below, align=center, text width=3.2cm] at (0,-0.1)
    {\STwo\\ Performance eval.\\ \footnotesize\itshape (hva måles)};
  \node[s3color, font=\bfseries, below, align=center, text width=3.2cm] at (2.4,-0.1)
    {\SThree\\ Reward systems\\ \footnotesize\itshape (hvordan belønnes)};
  % Komplementaritet-piler nederst
  \draw[<->, gray, dashed] (-2.1,-2.0) -- (-0.3,-2.0);
  \draw[<->, gray, dashed] (0.3,-2.0) -- (2.1,-2.0);
  \draw[<->, gray, dashed, bend right=25] (-2.1,-2.1) to (2.1,-2.1);
  \node[gray, font=\footnotesize\itshape] at (0,-2.6) {Komplementære -- endring i ett ben krever justering av de andre};
\end{tikzpicture}
\caption{\textbf{Den tre-benede skammelen} (BSZ kap.\ 11, slide 4). Organisasjonsarkitekturen hviler på tre komplementære søyler. Som en skammel: kapper du av ett ben (f.eks.\ desentraliserer beslutningsretten uten å tilpasse måling og belønning), velter konstruksjonen. Designet av hvert ben må passe til de to andre. [SLIDE]}
\end{figure}
