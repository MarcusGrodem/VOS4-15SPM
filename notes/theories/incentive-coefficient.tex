...
\begin{tikzpicture}[every node/.style={font=\small}]
  ...
  \node[draw, thick, rounded corners, fill=gray!10, font=\large] (F) at (0,0)
    {$\beta^* = \dfrac{\alpha}{\alpha^2 + r\,\sigma^2\,C''(e)}$};
...
\end{tikzpicture}
\caption{Faktorer som favoriserer høy $\beta$: høy $\alpha$ (innsats slår igjennom på output), lavt $\sigma^2$ (lite støy), lav risikoaversjon $r$ (agent tåler å bære risiko), og fleksibel innsats. Optimal $\beta^* = \alpha / (\alpha^2 + r\sigma^2 C''(e))$. [SLIDE -- BSZ kap.\ 15, slide 18]}
\end{figure}
