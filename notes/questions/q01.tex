...
\item \textbf{Cost center.} Fordel: enkelt å måle, klart ansvar. Ulempe: kostnadsminimering er ikke det samme som profittmaksimering -- kvalitet kan ofres for å pynte på enhetskosten. [SLIDE]
\item \textbf{Expense center.} Fordel: praktisk når output ikke kan måles (HR, R\&D, stab). Ulempe: evaluering blir subjektiv; \emph{empire building} (lederen jakter større budsjett og flere ansatte for prestisje); \emph{soft budget constraints} (Kornai: budsjettrammene er ikke ``harde'' -- overforbruk dekkes likevel av sentralt, så lederen mister disiplinen). [SLIDE]
\item \textbf{Revenue center.} Fordel: bevarer sentral priskontroll. Ulempe: når lederen kun maksimerer omsetning (MR$=0$ i stedet for MR$=$MC), gir hun for store rabatter eller feil produktmiks. [SLIDE]
\item \textbf{Profit center.} Fordel: utnytter lokal spesifik viden om kostnad og marked. Ulempe: eksternaliteter mellom divisjoner (én divisjons handlinger påvirker en annen -- f.eks.\ kannibalisering), transfer pricing-konflikter, og free-riding på fellesgoder (lederen kan slippe å betale full pris for konsernfunksjoner). [SLIDE/BOK]
\item \textbf{Investment center.} Fordel: kobler drift og kapitalbruk -- lederen ansvarliggjøres for hvor mye kapital som faktisk bindes. Ulempe: kortsiktige regnskapsmål (ROA -- Return on Assets) kan få lederen til å droppe gode langsiktige investeringer fordi ROA først synker; løsning er EVA (Economic Value Added, som korrigerer for kapitalkost). [SLIDE/BOK]
\end{itemize}
...
I Mærsk er Ocean-divisjonen og Logistics \& Services drevet som separate \emph{profit/investment centers} med egne P\&L (resultatregnskap). Power-to-X-satsingen er typisk et stort investeringsprosjekt der ansvarsenheten må evalueres som et investment center (kapitalkravet er enormt og avkastningen kommer langt frem i tid). Caset belyser EVA-problematikken: regnskapsmessig ROA i de første årene kan se elendig ut fordi kapitalbasen er stor og inntektene ikke har kommet ennå, mens nåverdien (NPV) er positiv. ROA-måltallet vil derfor straffe lederen for å gjøre noe som faktisk skaper verdi -- EVA korrigerer ved å belaste eksplisitt for kapitalkost og kreditere langsiktig verdiskaping. [CASE]
