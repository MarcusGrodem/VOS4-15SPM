...
Coase (1937): firmaet eksisterer fordi det er billigere å organisere noen transaksjoner via et hierarki (sjefen sier ``gjør dette'') enn via markedet (finn leverandør, forhandle, skriv kontrakt, overvåk). Firmaet erstatter altså markedstransaksjoner med hierarki når \emph{transaksjonskostnadene} -- søk, forhandling, skriving, overvåkning, håndheving -- er høye. Williamson (1985): valget \emph{make-or-buy} (lag det selv eller kjøp i markedet) avhenger av tre faktorer:
\begin{itemize}\setlength{\itemsep}{1pt}
\item \emph{Asset specificity}: hvor lite kan aktivumet brukes til andre ting? Høy spesifisitet $\Rightarrow$ hold-up-fare $\Rightarrow$ integrer.
\item \emph{Usikkerhet}: jo mer uforutsigbar framtiden er, jo vanskeligere er det å skrive komplette kontrakter $\Rightarrow$ integrer.
\item \emph{Frekvens}: ofte gjentatte transaksjoner gir argument for å bygge en intern løsning.
\end{itemize}
Grossman--Hart--Moore (GHM, ``property rights theory''): når kontrakter er ufullstendige, er det den som \emph{eier} aktivumet som bestemmer i situasjoner kontrakten ikke dekker -- altså har \emph{residuelle beslutningsrettigheter}. Eierskap = makt i ``hullene'' i kontrakten. Firmaets grense trekkes der marginal intern organiseringskostnad (byråkrati, agentkostnad) = marginal transaksjonskostnad (markedsbruk). [SLIDE]
...
BSZ kap.\ 19-modellen: oppstrøms monopolist $U$ (\emph{upstream}, f.eks.\ råvareprodusent) integrerer fremover til en av nedstrøms-aktørene $D_1$ (\emph{downstream}, foredler/distributør), og kontrollerer dermed tilgangen til inputfaktoren for rivalene $D_2, D_3$. $U$--$D_1$ kan da foreta \emph{foreclosure} -- bokstavelig ``utestengning'': nekte å selge input til rivalene eller sette så høy pris at de ikke kan konkurrere. En mildere variant er \emph{raising rivals' costs} -- bare gjøre det dyrere for konkurrentene. Resultatet: sluttforbrukerpris stiger, kvantum synker. Forutsetter oppstrøms monopol og at konkurransemyndighetene ikke griper inn. Chicago School-kritikken: monopolisten kan allerede ekstrahere overskudd (\emph{surplus}) gjennom inputprisen, så vertikal integrasjon tilfører ikke ny markedsmakt -- den endrer bare hvor profitten bokføres. [SLIDE/BOK]
