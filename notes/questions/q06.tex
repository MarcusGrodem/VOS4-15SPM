...
\begin{tikzpicture}[scale=1.0]
  ...
  % MC lineær: A + 2CD, A=1, stigning 0.5
  \draw[thick, red!70!black] (0,1) -- (7,4.5) node[right] {$\mathrm{MC}=A+2CD$};
  % D* der MB=MC: 1+0.5D=4 => D=6? med disse tallene topp ved D=6; juster:
  % La oss heller bruke A=1, helning 1 (2C=1, dvs C=0.5) => krysning ved D=3
  \draw[dashed] (3,0) -- (3,4);
  \filldraw[black] (3,4) circle (1.5pt);
  \node[below] at (3,0) {$D^{*}$};
  \node[left] at (0,4) {$B$};
  \node[left] at (0,1) {$A$};
  \node[above right] at (3,4) {\small Optimum: $D^{*}=\tfrac{B-A}{2C}$};
\end{tikzpicture}
