...
De fem incentivkonfliktene mellom eier og leder -- shirking, perks/lønn, risikoaversjon, korttidshorisont og overinvestering -- er de samme som i Q03. Det som skiller Q04 er \emph{forutsetningene}:
\begin{itemize}\setlength{\itemsep}{2pt}
\item Kontrakter er \emph{kostbare} å skrive, overvåke og håndheve (\emph{costly contracting}, motsatsen til Q03s costless-benchmark).
\item Informasjonen er \emph{asymmetrisk}: én part vet noe den andre ikke vet.
\end{itemize}
Konsekvens: kontrakten kan ikke lenger eliminere konfliktene -- vi havner i \emph{second-best}, en nest-beste-løsning der en restmengde tap aksepteres. Problemene deles i to grupper etter \emph{tidspunkt}:
\begin{itemize}\setlength{\itemsep}{2pt}
\item \textbf{Pre-kontraktuelt} -- \emph{adverse selection} (skjult \emph{type}, dvs.\ partneren skjuler hvem hun er). Håndteres med \emph{screening} (uinformert part gransker -- f.eks.\ due-diligence) og \emph{signaling} (informert part viser troverdig kvalitet -- f.eks.\ garantier, sertifisering).
\item \textbf{Post-kontraktuelt} -- \emph{moral hazard} (skjult \emph{handling}, dvs.\ etter kontraktinngåelse kan agenten skjule hva hun faktisk gjør). Håndteres med \emph{monitoring} (overvåkning), \emph{bonding} (agenten binder noe selv -- skin-in-the-game) og insentivlønn ($\beta > 0$).
\end{itemize}
En residual loss -- gjenværende effektivitetstap -- må alltid aksepteres. Når prinsipalen og agenten er to \emph{virksomheter} (interfirm contracting), løftes problemet til kontrakter om leveranser, partnerskap og felles investeringer. [SLIDE]
...
Før kontrakten signeres kjenner én part sin egen \emph{type} (kvalitet, kompetanse, risiko) bedre enn motparten -- \emph{hidden information}. I to-virksomhetsperspektivet: en teknologileverandør vet om elektrolyseteknologien faktisk er moden, kjøperen ikke. Akerlofs \emph{lemons}-mekanisme (1970, brukt-bil-markedet): når kjøperen ikke kan skille gode og dårlige biler, byr hun en gjennomsnittspris -- de gode selgerne trekker seg, gjennomsnittet faller, og til slutt sitter bare ``sitronene'' (skrapbilene) igjen. Løsninger: \emph{signaling} -- den informerte parten viser troverdig kvalitet via kostbare signaler som garantier, sertifiseringer eller eierandel i prosjektet; \emph{screening} -- den uinformerte parten gransker via due-diligence eller tilbyr kontraktsmenyer som får hver type til å avsløre seg selv (selvseleksjon); og reputasjon. [SLIDE]

\paragraph{\thref{moral-hazard}{Moral Hazard (post-kontraktuelt -- hidden action)}\quad\SThree}
Etter at kontrakten er signert kan agenten ta skjulte handlinger (\emph{hidden action}): shirke, kutte kvalitet, fortie problemer, prioritere egen profit over felles. Løsninger: \emph{monitoring} -- prinsipalen bruker ressurser på å observere (revisjon, KPI-oppfølging, rapportering); \emph{bonding} -- agenten forplikter selv noe av sin formue eller framtid (\emph{skin-in-the-game}: opsjoner som modnes over tid -- \emph{vesting}, eller \emph{claw-back}-klausuler som krever tilbakebetaling av bonus ved senere oppdaget feil); og variabel lønn $\beta > 0$. En \emph{residual loss} -- gjenværende effektivitetstap -- gjenstår alltid. Summen av monitoring, bonding og residual loss utgjør \thref{agency-costs}{agency costs}, totalkostnaden ved at agenten ikke handler perfekt i prinsipalens interesse. [SLIDE]
...
\begin{tikzpicture}[>=latex, every node/.style={font=\footnotesize}]
  \node[circle, draw, very thick, fill=gray!15, minimum size=2.4cm, font=\bfseries] (F) {Virksomheten};
  \node[draw, rounded corners, fill=blue!10, minimum width=2.1cm] (E) at (90:3.2)  {Eiere};
  \node[draw, rounded corners, fill=blue!10, minimum width=2.1cm] (K) at (30:3.6)  {Kunder};
  \node[draw, rounded corners, fill=blue!10, minimum width=2.1cm] (L) at (-30:3.6) {Leverandører};
  \node[draw, rounded corners, fill=blue!10, minimum width=2.1cm] (B) at (-90:3.2) {Banker / kreditorer};
  \node[draw, rounded corners, fill=blue!10, minimum width=2.1cm] (A) at (-150:3.6){Ansatte};
  \node[draw, rounded corners, fill=blue!10, minimum width=2.1cm] (R) at (150:3.6) {Regulator / stat};
  \foreach \n in {E,K,L,B,A,R} {
    \draw[<->, thick, gray!70] (F) -- (\n);
  }
  \node[font=\scriptsize\itshape, gray] at (0,-4.0) {hver pil = en (ufullstendig) kontrakt -- alle priset, alle forhandlet};
\end{tikzpicture}
\caption{Virksomheten som en \emph{nexus of contracts} (Jensen \& Meckling 1976) -- ingen ``ting'' i seg selv, men et knutepunkt der eiere, ansatte, kunder, leverandører, kreditorer og regulator er knyttet sammen gjennom (ufullstendige) kontrakter. Q04 fokuserer på principal-agent-kontrakten mellom to virksomheter under costly contracting. [SLIDE -- BSZ kap.\ 10]}
\end{figure}
