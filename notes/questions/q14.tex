...
Informativeness-prinsippet (Holmström 1979) sier at et signal $y$ bør inkluderes i kontrakten hvis og bare hvis det er statistisk informativt om agentens innsats. Med statistisk språk: signalet endrer \emph{posterior} for $e$ gitt det primære målet $Q$. På norsk: posterior er sannsynlighetsvurderingen din \emph{etter} at du har sett dataene -- altså, gjør $y$ at vi tror noe nytt om hvor hardt agenten har jobbet, gitt at vi allerede vet $Q$? Hvis ja, skal $y$ med i kontrakten. Prinsippet binder de fire delspørsmålene sammen: (a) controllability (kontrollerbarhet -- at agenten kan styre målet) er et \emph{spesialtilfelle} -- informativeness er bredere, fordi ukontrollerbare mål kan likevel være informative; (b) mer informative signaler forbedrer risikodelingen ved å senke effektiv støy; (c) presise mål (lav varians $\sigma^2$) er mer informative og bør vektes høyere ($\beta$ opp); (d) risikopremien $RP = \tfrac{r}{2}\beta^2\sigma^2$ -- prisen prinsipalen må betale for å lesse risiko på en risikoavers agent -- synker når signaler filtrerer bort støy. [SLIDE]

\subsection*{Relevante teorier (kort)}

\paragraph{\thref{informativeness-principle}{Informativeness-prinsippet}\quad\STwo/\SThree}
Holmströms tilstrekkelighetskriterium (på statistikkspråk: \emph{sufficient statistic}-betingelsen): inkluder signal $y$ i kontrakten hvis og bare hvis $f(Q|e,y)\neq f(Q|e)$ for noen $(Q,y)$. Med ord: legg til $y$ bare hvis sannsynlighetsfordelingen for $Q$ endrer seg når vi også vet $y$ -- ellers tilfører $y$ ingen informasjon utover det $Q$ allerede gir. Kontrakt utvides til $W=W_0+\beta_1 Q + \beta_2 y$, altså lønn med vekter på to ulike måltall. Effekten: lavere effektiv varians $\sigma^2_{\text{eff}} < \sigma^2$ (signalet filtrerer bort en del av støyen), dermed lavere risikopremie for gitt insentivnivå, eller sterkere insentiver for gitt risikopremie. \emph{Sufficient statistic}-poenget (en ``tilstrekkelig'' størrelse fanger all relevant informasjon): hvis $Q$ allerede fanger all informasjon om $e$, skal $y$ \emph{ikke} brukes -- ekstra mål gir bare ekstra støy. [SLIDE]

\begin{tcolorbox}[colback=blue!4, colframe=blue!50!black, boxrule=0.4pt, arc=2pt, left=6pt, right=6pt, top=4pt, bottom=4pt]
\textbf{Intuisjon:} $Q$ er alltid støyfullt. Legg til signal $y$ \emph{kun hvis} $y$ hjelper deg å skille innsats ($e$) fra flaks -- dvs.\ filtrerer bort noe av støyen. Konsekvens: $\sigma^2_{\text{eff}} \downarrow \Rightarrow$ presisjon $\uparrow$, kontrollerbarhet $\uparrow$, risikodeling bedres, $RP = \tfrac{1}{2} r \beta^2 \sigma^2_{\text{eff}} \downarrow$.\\[2pt]
Viktig: det er \emph{produktet} $\beta^2 \cdot \sigma^2$ som bestemmer kostnaden. Selv om $\beta^*$ stiger når $y$ legges til, faller $\sigma^2$ raskere $\Rightarrow$ nettokostnaden (risikopremien) faller. Relativ prestasjonsmåling (peer-gruppe) er klassisk eksempel -- filtrerer felles markedssjokk.
\end{tcolorbox}
...
Presisjon defineres som invers varians: høy presisjon $\Rightarrow$ lav $\sigma^2$ $\Rightarrow$ høyere $\beta^*$ via formelen \thref{incentive-coefficient}{$\beta^* = 1/(1+r\sigma^2 c''/\alpha^2)$}. Intuitivt: når målet er presist, kan man trygt vekte det tungt i kontrakten -- agenten påføres ikke unødig risiko. Risikopremien $RP = \tfrac{r}{2}\beta^2\sigma^2$ stiger med støyen $\sigma^2$, så upresise mål er dyre å bruke som insentivgrunnlag. Informativeness-prinsippet sier: legg til signaler som øker presisjonen (senker effektiv $\sigma^2$), og unngå signaler som bare tilfører støy. Relative prestasjonsmål -- der man måler agenten mot en peer-gruppe i stedet for mot et absolutt mål -- er det viktigste eksemplet (\thref{absolute-vs-relative-performance}{rTSR}, dvs.\ \emph{relative Total Shareholder Return}, måler aksjeavkastning mot konkurrenter): peer-avkastning filtrerer felles markedssjokk og øker presisjonen av det individuelle signalet. [SLIDE/BOK]
...
\begin{figure}[H]
  ...
  \caption{Relativ prestasjonsmåling som turnering: informativeness-prinsippet begrunner bruk av peer-signaler for å filtrere felles sjokk. (BSZ kap.\ 14, slide 13) [SLIDE]}
\end{figure}
