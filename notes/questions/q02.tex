...
\emph{Transfer pricing} er prisen som settes når én divisjon i konsernet leverer internt til en annen divisjon. Den teoretisk korrekte prisen er lik selgerens \emph{opportunity cost} (alternativkostnad) -- altså det selgeren mister ved \emph{ikke} å selge eksternt i stedet. Spørsmålet skiller mellom to forutsetninger:
\begin{itemize}\setlength{\itemsep}{2pt}
\item (a) \emph{Costless information}: toppledelsen kjenner alle kostnader gratis og kan sette riktig pris direkte. Resultatet blir konsernoptimal mengde -- en \emph{førstebeste}-løsning.
\item (b) \emph{Asymmetrisk informasjon}: divisjonene sitter på lokal kunnskap og har insentiv til å rapportere kostnader strategisk. Da finnes \emph{ingen} førstebeste prisingsmetode.
\end{itemize}
I (b) må man velge mellom fem praktiske metoder -- market-based (markedspris), marginal cost (MC), full cost (full kostnad), full cost plus markup (full kostnad pluss påslag) og negotiated (forhandlet) -- og valget blir en avveining mellom \thref{decision-management-control}{\emph{decision management}} (sette riktig pris for beslutningen om mengde) og \emph{decision control} (sette riktig pris for insentiv og rettferdig profittfordeling). Samme tall kan ikke alltid brukes til begge formål. [SLIDE]

\subsection*{Relevante teorier (kort)}

\paragraph{\thref{transfer-pricing}{Transfer Pricing}\quad\SOne/\STwo/\SThree}
Oppstår nødvendigvis i konserner med profit- og investmentcentre (divisjoner som måles på egen profitt/avkastning) og interne leveranser. Optimal pris er lik opportunity cost (alternativkostnad). I praksis betyr det: marginalkostnad (MC) hvis selgerdivisjonen har \emph{ledig kapasitet} (det koster bare den ekstra produksjonen), eller MC pluss fortrengt dekningsbidrag hvis selgeren er kapasitetsbundet (da må prisen også dekke det tapte salget eksternt). Transferprisen påvirker tre ting samtidig: divisjonens profitt (\STwo\ -- prestasjonsmåling), lederens insentiv og bonus (\SThree), og hvilke produksjons- og investeringsbeslutninger som faktisk treffes (\SOne). De fem metodene i pensum er ulike praktiske tilnærminger til denne ideelle prisen. [SLIDE]
